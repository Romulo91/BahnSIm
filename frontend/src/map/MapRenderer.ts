...
export class MapRenderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    window.addEventListener('resize', () => {
      this.resizeCanvas()
      if (this.tracks.length > 0) this.render()
    })
  ...
  //   Das Spread-Operator (...)  lädt alle Werte als Funktionsargumente auf den
  ...
  render(): void {
    const { width, height } = this.canvas
    console.info(`Render ${this.tracks.length} Tracks auf ${width}×${height}px`)

    // Hintergrund schwarz füllen – überschreibt das vorherige Frame vollständig.
    ...
    // Bounding Box einmal berechnen – gilt für alle Tracks gleichermaßen.
    ...
    const project = makeProjection(bbox, width, height, 40)
